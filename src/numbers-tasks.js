...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const diffBetweenX = x2 - x1;
  const diffBetweenY = y2 - y1;
  return Math.sqrt(diffBetweenX * diffBetweenX + diffBetweenY * diffBetweenY);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cos = multSum / (value1 * value2);
...
function roundToPowerOfTen(num, pow) {
  ...
  const round = Math.round(num / powOfTen);
  return round * powOfTen;
}
